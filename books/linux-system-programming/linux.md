...
- Linuxのより高度なI/Oシステムコール
- 1. scatter-gather I/O（前章のリニアI/Oとの対比でベクタI/Oとも）
	- 1データストリームへ複数のバッファを一度のシステムコールで書き込む/読み込む
...
- writev(), readv()は複数のiovec構造体の情報に従ってシーケンシャルに読み書きをするだけなので、forで実装できる
	- が、実際はシステムコールレベルの話なのでこんな実装が存在するわけではない
	- write(), read()はベクタI/O readv(), writev() でセグメント数1の場合と等価
- 2. event poll (epoll)
	- poll(), select() は対象のfdのセットをすべてシステムコールに渡していた
		- 何百、何千のfdをシステムコール発行のたびに逐一調べるのか、と言う問題になる
- epoll() は、poll()システムコールの仕事を3分割してfdの管理をいい感じにしたもの
	...
	3. epoll_wait(epfd, epoll_event, maxevents, timeout)
		- 設定済fdについて、イベント発生を待つ
		- 発生したイベントはepoll_eventに入る
- epoll_ctlでEPOLLETを指定するとエッジトリガでfdを調べる（デフォはレベルトリガ）
...
- Unix
	...
	- 読み込み：シンクロナス＆シンクロナイズド
		- 読み込みのシンクロナイズド＝ベージキャッシュ内のデータは常にディスク上のデータと一致する（またはそれより新しい状態である）ことが保証
- 非同期（アシンクロナス）I/O
...
- 以下を満たすスレッド実装を使えばこの2つを実現できる
	- すべてのI/Oを担当するワーカスレッドをあらかじめ作成
	- ワークキューの専用インタフェースを作成
		- 渡されたI/O要求に一意に対応するI/Oディスクリプタを返す
		-	 ワーカスレッドはキューの先頭からI/O要求を取り出して、I/Oを発行して、完了を待つ
	- I/Oが完了したら戻り値、errno、読み取ったデータなどを結果キューにつなぐ
	- 結果キューからI/O結果を取り出すインタフェースを提供
...
- 仮にスケジューラが”常に”ブロックをソートして処理していれば、遠いブロックがいつまでも読めず、読み取りレイテンシ的にまずい
	- 初期のめちゃ単純な対策はLinusエレベータ
	- 一定時間以上残っているI/O要求はソートせずにキューへ追加
- Deadline I/O スケジューラ
	- エレベータをもう少しシステマティックに賢く実装
	- I/O要求キューに加えて、読み取りFIFOキューと書き込みFIFOキューを追加して情報を記録・管理
	- 要求を発行順序でソート、制限時間を設定
...
- CFQ (Complete Fair Queuing) I/O スケジューラ
	- バランス良い、おすすめ
	- プロセスごとに独自のキュー（タイムスライスつき）を用意して、各プロセスのキューをラウンドロビンで処理していく
...
- カーネル側ではスケジューラでディスクシークを抑えている。アプリケーション我でできることは？
	...
	- アプリ側での要求ソートはパス名、inode番号、物理ディスクブロック番号を参照しながら行う
		- パス名
